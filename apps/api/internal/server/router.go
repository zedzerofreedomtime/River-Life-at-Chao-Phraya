...
func (s *Server) setUserSession(c *gin.Context, userID string) error {
	t := service.Token()
	if err := s.Redis.Set(c.Request.Context(), "riverlife:user-session:"+service.Hash(t), userID, 7*24*time.Hour).Err(); err != nil { return err }
...
func (s *Server) googleCallbackURL() string { return strings.TrimRight(s.WebOrigin, "/") + "/api/v1/auth/google/callback" }
...
func (s *Server) Router() *gin.Engine {
	...
	api.POST("/auth/otp/request", s.limiter(5), func(c *gin.Context) {
		var in struct { Email string `json:"email"`; Name string `json:"name"` }
		if c.ShouldBindJSON(&in) != nil || len(strings.TrimSpace(in.Name)) > 120 { bad(c); return }
		...
		if err = s.Redis.Set(c.Request.Context(), "riverlife:otp:"+service.Hash(email), encoded, 10*time.Minute).Err(); err != nil { fail(c, err); return }
		if !s.Demo {
	...
	api.POST("/auth/otp/verify", s.limiter(8), func(c *gin.Context) {
		var in struct { Email string `json:"email"`; Code string `json:"code"` }
		if c.ShouldBindJSON(&in) != nil || len(in.Code) != 6 { bad(c); return }
		email, ok := validEmail(in.Email)
		if !ok { bad(c); return }
		key := "riverlife:otp:"+service.Hash(email)
		raw, err := s.Redis.Get(c.Request.Context(), key).Bytes()
		if err != nil { c.AbortWithStatusJSON(401, gin.H{"error": "รหัส OTP หมดอายุหรือไม่ถูกต้อง"}); return }
		...
		if json.Unmarshal(raw, &challenge) != nil || subtle.ConstantTimeCompare([]byte(challenge.CodeHash), []byte(service.Hash(in.Code))) != 1 {
			c.AbortWithStatusJSON(401, gin.H{"error": "รหัส OTP หมดอายุหรือไม่ถูกต้อง"}); return
		}
		if err = s.Redis.Del(c.Request.Context(), key).Err(); err != nil { fail(c, err); return }
		user, err := s.Service.UpsertUser(c.Request.Context(), email, challenge.Name, "email", email)
		if err != nil { fail(c, err); return }
		if err = s.setUserSession(c, user.ID); err != nil { fail(c, err); return }
		c.JSON(200, user)
	})
	api.GET("/auth/me", func(c *gin.Context) {
		id, ok := s.userID(c)
		if !ok { c.JSON(200, gin.H{"authenticated": false}); return }
		user, err := s.Service.User(c.Request.Context(), id)
		if err != nil { fail(c, err); return }
		c.JSON(200, gin.H{"authenticated": true, "id": user.ID, "name": user.Name, "email": user.Email})
	})
	api.POST("/auth/logout", func(c *gin.Context) {
		if t, err := c.Cookie(userCookie); err == nil { _ = s.Redis.Del(c.Request.Context(), "riverlife:user-session:"+service.Hash(t)).Err() }
		c.SetSameSite(http.SameSiteLaxMode)
		c.SetCookie(userCookie, "", -1, "/api/v1", "", s.CookieSecure, true)
		c.Status(http.StatusNoContent)
	})
	api.GET("/auth/google/start", func(c *gin.Context) {
		if s.GoogleClientID == "" || s.GoogleClientSecret == "" { c.AbortWithStatusJSON(503, gin.H{"error": "Google Sign-In ยังไม่ได้ตั้งค่า"}); return }
		state := service.Token()
		if err := s.Redis.Set(c.Request.Context(), "riverlife:google-state:"+service.Hash(state), "1", 10*time.Minute).Err(); err != nil { fail(c, err); return }
	...
	api.GET("/auth/google/callback", func(c *gin.Context) {
		...
		if state == "" || code == "" || s.Redis.Del(c.Request.Context(), key).Val() != 1 { c.Redirect(http.StatusFound, strings.TrimRight(s.WebOrigin,"/")+"/auth?error=google"); return }
		...
		if err != nil { fail(c, err); return }; defer response.Body.Close()
		var tokens struct { AccessToken string `json:"access_token"` }
		if response.StatusCode != 200 || json.NewDecoder(response.Body).Decode(&tokens) != nil || tokens.AccessToken == "" { c.Redirect(http.StatusFound, strings.TrimRight(s.WebOrigin,"/")+"/auth?error=google"); return }
		...
		if err != nil { fail(c, err); return }; defer infoResponse.Body.Close()
		var info struct { Subject string `json:"sub"`; Email string `json:"email"`; Name string `json:"name"`; EmailVerified bool `json:"email_verified"` }
		if infoResponse.StatusCode != 200 || json.NewDecoder(infoResponse.Body).Decode(&info) != nil || info.Subject == "" || !info.EmailVerified { c.Redirect(http.StatusFound, strings.TrimRight(s.WebOrigin,"/")+"/auth?error=google"); return }
		user, err := s.Service.UpsertUser(c.Request.Context(), info.Email, info.Name, "google", info.Subject)
		if err != nil || s.setUserSession(c, user.ID) != nil { c.Redirect(http.StatusFound, strings.TrimRight(s.WebOrigin,"/")+"/auth?error=google"); return }
